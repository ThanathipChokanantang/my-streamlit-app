...
if not api_key:
    ...
    try:
        ...
        st.header("1. ระบุเหตุการณ์และสถานที่ (ภาษาไทย)")
        ...
        if st.button("🚀 สรุปสถิติและวิเคราะห์", type="primary"): 
            if event_type_th and location_th:
                ...
                st.success(f"✔️ แปลแล้ว: เหตุการณ์='{event_type_en}', สถานที่='{location_en}'")
                
                # --- ขั้นตอนที่ 1: ค้นหาข้อมูลดิบ (Tool Use) ---
                with st.spinner(f"⏳ ขั้นตอนที่ 1: กำลังค้นหาข้อมูลดิบ (ภาษาอังกฤษ) เกี่ยวกับ '{event_type_en}' ใน '{location_en}'..."):
                ...
                with st.spinner("⏳ ขั้นตอนที่ 2: กำลังวิเคราะห์และสกัดข้อมูลเป็น JSON Array (บังคับสรุปรายละเอียดเป็นภาษาไทย)..."):
                ...
                if json_output:
                    ...
                    try:
                        ...
                        # กำหนดคอลัมน์ที่จะแสดงใน DataFrame
                        display_cols = [
                            'ลำดับ_เหตุการณ์',
